...
fn add_proptest_dependency(doc: &mut DocumentMut) {
    ...
    // Activate the implicit `proptest` feature from the optional dep in test-support.
    // Using bare `proptest` (not `dep:proptest`) preserves Cargo's implicit feature
    // so `#[cfg(feature = "proptest")]` in source (e.g. gpui/src/color.rs) resolves.
    if let Some(features) = doc.get_mut("features") {
        if let Some(table) = features.as_table_like_mut() {
