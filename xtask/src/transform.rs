...
fn add_proptest_dependency(doc: &mut DocumentMut) {
    ...
    // Add proptest to test-support feature. Use the bare name (not "dep:proptest")
    // so cargo's implicit feature for the optional dep is created — source code
    // gates with #[cfg(feature = "proptest")] depend on it.
    if let Some(features) = doc.get_mut("features") {
        if let Some(table) = features.as_table_like_mut() {
