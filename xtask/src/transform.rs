...
fn add_proptest_dependency(doc: &mut DocumentMut) {
    ...
    // Define an explicit `proptest` feature that activates the optional dep.
    // Source code uses `#[cfg(feature = "proptest")]`, and pushing `dep:proptest`
    // anywhere would suppress Cargo's auto-feature, leaving that cfg unrecognized
    // (a hard error under `-Dwarnings`). Have test-support enable the feature.
    if let Some(features) = doc.get_mut("features") {
        if let Some(table) = features.as_table_like_mut() {
            if !table.contains_key("proptest") {
                let mut proptest_arr = toml_edit::Array::new();
                proptest_arr.push("dep:proptest");
                table.insert("proptest", Item::Value(Value::Array(proptest_arr)));
            }
            if let Some(test_support) = table.get_mut("test-support") {
                if let Some(arr) = test_support.as_array_mut() {
                    // Drop any pre-existing `dep:proptest` entry — we want the
                    // bare `proptest` feature reference instead.
                    let entries: Vec<_> = arr.iter().cloned().collect();
                    let mut new_arr = toml_edit::Array::new();
